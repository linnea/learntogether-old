...
clean: db_drop
	...
	bower cache clean >>log/cleanup.log 2>&1 && ([ $$? -eq 0 ] && echo "success!") || echo "failure!"

db_build: ;@echo "Building database and logging to log/db.log...";
	@echo `date` > log/db.log
	@echo "Creating database..." | tee -a log/db.log
	@psql -h "$(db)" -f scripts/createDatabase.sql >>log/db.log 2>&1 && ([ $$? -eq 0 ] && echo "success!") || echo "failure!" | tee -a log/start.log
	@echo "Creating tables..." | tee -a log/db.log
	@psql -U root -d learntogether -h "$(db)" -f scripts/createTables.sql >>log/db.log 2>&1 && ([ $$? -eq 0 ] && echo "success!") || echo "failure!" tee -a start.log

db_drop: ;@echo "Destroying database and logging to log/db.log...";
	@echo `date` > log/db.log
	@psql -U root -d postgres -h "$(db)" -f scripts/deleteDatabase.sql >>log/db.log 2>&1 && ([ $$? -eq 0 ] && echo "success!") || echo "failure!"
